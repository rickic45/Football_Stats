...
async function renderInfo(d, teamSlug) {
  ...
  const favRes = await fetch(`${API_BASE}?url=favorites`);
...
async function toggleFavorite(league, team, btn) {
  ...
  // Se è preferita, rimuovi (DELETE), altrimenti aggiungi (POST)
  const metodo = isPreferita ? 'DELETE' : 'POST';
  
  const res = await fetch(`${API_BASE}?url=favorites`, {
    method: metodo,
